...
- ❌ **Do not edit** files under `force-app/` (Apex, LWC, objects, flows, etc.)
  directly — not with Edit, not with Write, not with Bash.
...
- ✅ For any code task, launch the appropriate subagent via the `Agent` tool:
  - `subagent_type: "spec_author"` → drafts
    ...
    `"sdd": true`.
  - `subagent_type: "implementer"` → writes metadata and tests for **one**
    feature whose spec is already approved (`in_progress`).
...
3. Apply the escalation table and the SDD flow from `.claude/agents/leader.md`.
...
return only the reference to you, not the content. See `.claude/agents/leader.md`
...
- Conceptual questions or repo exploration (read-only) → answer directly, no
  subagents.
- Changes outside `force-app/` (docs, configuration, `progress/`) → you may edit
  them yourself.
